 ...
 <?php
    ...
    $sqlCommand = "SELECT * FROM customer WHERE username = '$usernameData' AND secret='$passwordData';";
...
<div id="logout">
    ...
    <div class="container">
		...
		<?php 
			if (isValidUser()) {
				echo "<script>loginCookie();</script>";
				echo "<p>Thanks for logging in!</p><br />
				<form action='search_filter.php'>
				<button type='submit' class='btn btn-primary submitbtn'>CONTINUE</button>
				</form>";
			} else { 
				echo "<p>Your credentials weren't located in our database. Please try again</p>
				<form action='login.php'>
				<button type='submit' class='btn btn-primary submitbtn'>BACK</button>
			    </form>";
			}
			?>
		</div>
